...
const getExpectedTransformedData = () => ({
            pr: {
...
describe('test tranforms!', () => {
    ...
    it('test transform', () => {
        ...
        const expectedData = getExpectedTransformedData();

        expect(tranformedData).toEqual(expectedData);
    });

    it('test transform with replyto', () => {
        const comment = getCompleteComment();
        comment.fields.replyto = "previous-comment-123";

        const tranformedData = transformData(comment);

        const expectedData = getExpectedTransformedData();
        expectedData.pr.path = '_data/comments/blog-post/previous-comment-123entry1522076160000.yml';
        expectedData.comment.replyto = "previous-comment-123";
